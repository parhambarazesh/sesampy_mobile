...
import pprint
from core.jsonformat import format_object, FormatStyle
import simplejson as json
from core.connector_cli.connectorpy import *
from core.connector_cli.oauth2login import *
from core.connector_cli.tripletexlogin import *
...
class SesamNode:
    ...
    def put_config(self, config, force=False):
        self.logger.info("PUT config to %s" % self.node_url)
        self.api_connection.upload_config(config, force=force)

    def put_env(self, env_vars):
        self.logger.info("PUT env vars to %s" % self.node_url)
        self.api_connection.put_env_vars(env_vars)

    def get_env(self):
        self.logger.info("GET env vars from %s" % self.node_url)
        return self.api_connection.get_env_vars()

    def get_system(self, system_id):
        self.logger.info("Get system '%s' from %s" % (system_id, self.node_url))
        try:
    ...
    def add_system(self, config, verify=False, timeout=300):
        self.logger.info("Add system '%s' to %s" % (config, self.node_url))
    ...
    def add_systems(self, config):
        self.logger.info("Add systems '%s' to %s" % (config, self.node_url))
        return self.api_connection.add_systems(config)

    def remove_system(self, system_id):
        self.logger.info("Remove system '%s' from %s" % (system_id, self.node_url))
        try:
...
class SesamCmdClient:
    ...
    def authenticate(self):
        ...
        if os.path.isfile("manifest.json"):  # If manifest.json is in working directory
            self.args.connector_manifest = "manifest.json"
                ...
                os.path.join(args.connector_dir, "manifest.json")):  # If manifest.json is in connector directory
        ...
        if "auth_variant" in connector_manifest and connector_manifest["auth_variant"].lower() == "tripletex":
            ...
            if self.args.client_id is None or self.args.client_secret is None:
                logger.error(
                    "Missing client_id and/or client_secret. Please provide them in .authconfig or as arguments.")
                sys.exit(1)
            result=login_via_oauth(self.sesam_node, self.args)
            print("result",result)
        else:
            pass
                    ...
                    try:
                        ...
                        if entities_json is not None:
                            ...
                            try:
                                ...
                                self.logger.info(f"Failed to delete dataset {pipe_id}. It probably doesn't exist, "
    ...
    def download(self):
        if self.args.is_connector:
            if not os.path.isdir(os.path.join(self.args.connector_dir, self.args.expanded_dir)):
                logger.warning(
                    "Expanded directory '%s' does not exist. creating the directory." % self.args.expanded_dir)
    ...
    def load_test_specs(self, existing_output_pipes, update=False):
        ...
        for filename in glob.glob("expected%s*.test.json" % os.sep):
            ...
            self.logger.info("Pipe id for spec '%s' is '%s" % (filename, pipe_id))
...
def start_cli(args_dict):
    ...
    if args.verbose:
        ...
        # remove the old log file (core/output.log) and add a new one
        if os.path.isfile(os.path.join(BASE_DIR, "core/output.log")):
            os.remove(os.path.join(BASE_DIR, "core/output.log"))
        log_file = os.path.join(BASE_DIR, "core/output.log")
    ...
    logger.warning(f"CURRENT PATH1: {os.getcwd()}")
    if command not in ["authenticate", "upload", "download", "status", "init", "update", "verify", "test", "run",
                       "wipe",
